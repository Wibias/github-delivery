...
- Optional Windows Authority **PR sessions**: after one Hello approval, later exact-scope `push_code` and `merge_pr` batches on one allowlisted repo, one PR, and one head branch can skip repeated Hello for 5, 15, 30, or 60 minutes (`approvalMethod: pr_session`). Branch leases stay `push_code` only for 1–10 minutes. Comments, human replies, close, and delete still need Hello (PR #348).
...
- Router merge intent is explicit: `merge it` / `ship it` prefixes, negated merge, and attributed GitHub text such as `comment says: merge it` are not user merge authority (PRs #321, #322, #347).
...
- Merge grants bind the approved base; `close_linked_issue` binds the governing PR (PRs #335, #336).
...
- Install, update, and Authority cutover keep the previous skill/broker when a step fails: staged installer payloads, dist-not-cwd identity, exclusive reconcile/hooks locks, skill-backup restore, broker keep-alive, Startup-shortcut autostart, and bundle-root fallback (PRs #314, #326–#332).
...
- Orphan-workflow cleanup rechecks ref SHAs before each delete. Behavioural eval scores bind to execution traces (PRs #333, #334).
