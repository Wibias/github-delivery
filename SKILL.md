...
12. Merge-ready paths (`fix-pr-bots`, create-PR, full-review when posting merge-ready) **must** run own **bug + security + Spec/Standards** — not bots-only. Explicit **security review** workflow: mandatory coverage matrix + subagent + secrets scan (see `security-review.md`) — no shallow “no issues” without evidence. Other PR flows: security cue → ask. Public disclosure always; changelog/commit/semver → `git-workflow-and-versioning`; final evidence sweep before ready claims.
