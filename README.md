...
`github-delivery` turns natural-language requests into evidence-backed GitHub workflows: PRDs, issue research, implementation, deep review, CI, fixes, stacks, verified merges, and verified stable self-update. v0.8.1 improves the Windows Authority approval experience and Codex lifecycle-hook stall recovery: approval windows stay foregrounded with fixed-shell scrolling and exact branch context, temporary exact repo + branch grants can last up to 10 minutes, Control Center activity stays bounded, and narration-only Stop recovery gets up to three chances to reach a real tool boundary before failing closed.
...
- A lifecycle-hook narration recovery remains active even if the immediate follow-up text is too short to independently trip the repeat detector. It clears at `PreToolUse`, gets **three corrective continuations by default**, and otherwise fails closed after the bounded recovery is exhausted.
