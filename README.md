...
On a fresh machine, the guided flow installs the verified release, starts the Windows Authority host in the notification area when accepted, and registers it to start automatically when the current Windows user signs in. With an existing valid installation, it offers **Update / Repair setup / Exit** rather than silently reinstalling or updating. Use `npx github-delivery start` to launch the approval GUI again and repair the login auto-start registration. If loop interruption is still inactive after a guided install, the CLI reports that GitHub Delivery has not verified Codex hook trust for the installation. If the exact unchanged definitions are already trusted in Codex, they do not need to be trusted again; otherwise review them in `/hooks`, then rerun `npx github-delivery setup`.
