...
`github-delivery` turns natural-language requests into evidence-backed GitHub workflows: PRDs, issue research, implementation, deep review, CI, fixes, stacks, verified merges, and verified stable self-update. v0.8.2 tightens repeated Codex watchdog stalls after real DeepSeek V4 Flash incidents: protected-stream active-work budgets drop to 4k/8k characters and 1,024/2,048 output tokens, severe hook-mode recoveries stop after the recovery tool before another model response, same-turn probation hard-stops a second narration stall, and the six-clause imminent-tool threshold is retained to avoid legitimate tool-start false positives.
...
Trusted lifecycle hooks use turn-scoped state, keep evidence reads/searches from resetting the no-progress detector, warn after **8 consecutive evidence attempts** without execution/state progress, and deny the **12th** supported evidence attempt until the turn makes real progress. Exact duplicate reads, rapid repeated polls, and supported semantically covered evidence are blocked independently. Common workflow forms are classified accordingly: assignment-prefixed PowerShell reads such as `$c=Get-Content ...` consume the evidence budget, while Bun validation commands such as `bun test` and `bun run test|check|lint|build|typecheck|verify` count as execution progress and reset the consecutive-evidence streak. Successful `PostToolUse` results are never replaced or truncated by the generic watchdog. When a completed assistant or subagent message stalls in narration instead of reaching the selected tool, the Stop recovery obligation persists across short follow-up messages and allows up to **three corrective continuations by default**; it clears only at a real `PreToolUse` boundary and otherwise fails closed after the bounded recovery is exhausted. Malformed tool-protocol stalls still hard-stop immediately.
...
| **Agents can overbuild a correct-but-expensive solution.** | Non-trivial implementation first applies the minimal-solution contract: understand the real flow, then prefer repository reuse → standard library/runtime → native platform → an already-installed dependency → the minimum custom code that satisfies the full contract. |
| **Hard bugs invite anchoring and random fix attempts.** | Difficult diagnoses build a tight red-capable symptom signal, minimize the reproducer, rank falsifiable hypotheses, and change one controlled variable at a time before committing a production fix. |
...
| `simplify PR #42 without changing behavior` | Explicit-only, behavior-preserving simplification plus mandatory re-review |
...
|---|---|---|
| **Product / issue intake** | PRDs, breakdowns, triage, triage attention inbox, QA intake, refactor plans | `references/issue-workflows.md` |
...
| **Bug review** | Evidence-ranked adversarial bug hunt; difficult diagnoses use a tight red-capable feedback loop and falsifiable hypotheses | `references/bug-review.md` + `references/bug-hunt-method.md` |
...
| **Design quality** | Advisory happy-path, boundary, abstraction/deletion cost, domain/state, seam, locality/leverage, and evidence-vs-complexity review | `references/design-quality.md` |
...
| **Change execution** | Safe internal-API migrations, expand-contract waves, mechanical sweeps, codemod decisions, and verifiable change units | `references/change-execution.md` |
| **Completion evidence** | Prove durable completion/count/coverage/absence claims from fresh authoritative evidence | `references/completion-claims.md` |
...
- Lifecycle-hook Stop recovery remains active across short no-progress follow-up messages and permits up to three corrective continuations by default until a real `PreToolUse` boundary is reached; exhausting that bounded recovery fails closed.
...
Nine companion contracts keep implementation, review, bug-fix, migration, and completion evidence concrete without weakening the existing gates:

- [`references/minimal-solution.md`](references/minimal-solution.md) keeps non-trivial implementation and refactor planning on the smallest complete solution that fits the actual repository: reuse existing capability first, then standard library/runtime, native platform, an already-installed dependency, and only then the minimum custom code. It never authorizes removing required validation, security, accessibility, compatibility, lifecycle, observability, or evidence.
...
The bug axis uses a built-in **Finder → Challenger → Arbiter** method. Static-analysis leads and tool-free heuristics feed finding cards with explicit evidence and a Gate 0 impact bar. Coverage is reported honestly as `confirmed`, `dismissed`, `manual-review`, or `unreviewed`; partial coverage is never presented as clean. When a difficult bug's root cause is not already established by direct code evidence or an existing narrow regression check, diagnosis first builds a red-capable signal for the exact symptom, minimizes the reproducer, then tests ranked falsifiable hypotheses one controlled variable at a time.
...
The pre-open gate is **post-implementation and pre-publication**. It cannot become a research loop that prevents the first implementation commit. Completed issue research is reused when the relevant issue/development state has not changed. Before a non-trivial implementation is chosen, `references/minimal-solution.md` requires enough real-flow understanding to prefer existing repository capability, standard/runtime facilities, native platform features, or already-installed dependencies over unnecessary custom machinery.
...
A candidate must preserve APIs, errors, ordering, concurrency, side effects, persistence, compatibility, validation, authorization, security, CI/evidence boundaries, and other material behavior. Advisory deletion-test, interface/test-surface, real-seam, leverage, and locality heuristics help find worthwhile candidates, but do not override the behavior-preservation contract or repository architecture.
...
| **Codex lifecycle hooks** | The normal install configures `PreToolUse`, `PostToolUse`, `UserPromptSubmit`, `Stop`, `SubagentStop`, and `SessionEnd`. After Codex's explicit non-managed hook trust review, supported tool activity is tracked per turn, exact duplicate reads/polls and already-covered semantic evidence can be blocked, evidence does not reset narration history, the 8th consecutive evidence attempt warns, and the 12th is denied until execution/state progress occurs. Successful tool output is preserved; oversized subagent briefs remain bounded. A narration stall keeps a corrective obligation active across short follow-up messages and gets up to three continuations by default to reach a real `PreToolUse` boundary before failing closed. After a severe recovered stall of at least 8,000 characters, the already-selected recovery tool still runs, then `PostToolUse` returns `continue: false` and the model is quarantined for the task; smaller recovered stalls keep per-turn probation so a second fresh narration stall hard-stops instead of restarting recovery. Repeated grid or malformed tool-protocol output still hard-stops immediately, and the offending model is quarantined across turns and `SessionEnd` so a resume with the same model is blocked before inference until the model is changed. |
...
- A lifecycle-hook narration recovery remains active even if the immediate follow-up text is too short to independently trip the repeat detector. It clears at `PreToolUse`, gets **three corrective continuations by default**, and otherwise fails closed after the bounded recovery is exhausted. A severe recovered stall (>= 8,000 characters) still runs the selected recovery tool once, then stops before another model response and quarantines the model for the task; a smaller recovered stall keeps per-turn probation so a second fresh narration stall hard-stops instead of opening a new recovery cycle.
- Protected stream active-work defaults warn/hard-stop at **4k/8k generated characters** and **1,024/2,048 generated output tokens** since the last real progress. Six imminent execution clauses without a tool start or two malformed protocol-emission chunks are hard stalls. Exact/low-novelty detection may interrupt earlier.
...
Trusted hooks provide turn-scoped supported-tool guardrails, including duplicate/semantic evidence protection, the 8/12 evidence-exploration budget, and bounded narration recovery that requires a real `PreToolUse` boundary within three corrective continuations by default. They still cannot interrupt assistant text before a local tool boundary. For in-flight generated-text/tool-emission activity visible through App Server, use the protected launcher:
...
- temporary exact repository + branch grants selectable from **1 through 10 minutes**;
...
The Control Center has a functional **Settings** destination for **Off**, **Sensitive actions** (recommended), and **Every GitHub write**. It writes the same persistent `authorityMode` preference as the CLI and shows stored/effective mode plus Authority version/source status. If the preference file does not exist yet, **Sensitive actions** (`high-assurance`) is the default rather than silently disabling trusted authority. Recent activity / Audit trail is capped at 420 px with internal scrolling. Approval windows keep repository, security, branch controls, and approval buttons fixed while only long proposed-action content scrolls; they remain above other windows while waiting, show the exact branch-driving fields, and bind Windows Hello to the approval window while keeping TPM/TBS failures fail-closed with retry/sign-in recovery.
