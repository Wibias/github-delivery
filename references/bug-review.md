...
| **edge_cases**      | Null/empty/partial collections; off-by-one bounds; races/TOCTOU on shared state; partial failure mid-batch; **mixed-state aggregates** (doc says "every X or Y", code requires "all X or all Y" — mixed distributions escape); **recursive/re-entrant lookup must terminate** (self-recursion on a resolved target = stack overflow); **lock/contention → caller contract** (below) |
...
| **determinism_metrics** | Aggregation/truncation must be deterministic (stable tie-break keys); **filter before LIMIT/truncation** (applying LIMIT before the candidate filter loses samples non-deterministically); **one decision reads one clock snapshot** (multiple `Date.now()` reads in a single evaluation are a race — freeze one `now`); grouping keys include every distinguishing dimension (e.g. profile revision); output metric names must not promise more than the value means |
...
6. **Unknown must not outrank measured** — when scoring blends evidence, an unknown value must get a neutral score, not the raw priority score; otherwise a candidate with unknown health/eligibility beats one with measured-but-imperfect evidence.
7. **Hard requirements gate on the same normalized value the scoring uses** — a hard eligibility requirement (min-threshold, max-usage, min-capacity, min-version) must be evaluated against the **same normalized/derived value the scorer/classifier uses**, not a raw sub-field that can be structurally absent. Concretely: when a derived score maps a signal to a canonical value (`exhausted → 0`, `unknown → null`), the requirement must use that canonical value — a record where the signal is present-but-zero (e.g. exhausted) but the raw field is absent must evaluate as **zero (ineligible)**, never as "no evidence → pass." The raw-field check and the normalized check must agree on absent vs malformed vs zero.
8. **Hard limits come from the configured policy, not caller-supplied evidence** — an evaluator that compares `estimatedUsd > limitUsd` only when the caller echoed a `limitUsd` into the evidence object silently skips the hard ceiling for every caller that omits it (e.g. dry-run evidence with just `{ estimatedUsd }`). Enforce `profile.limits.maxEstimatedCostUsd` (or the profile's own threshold) directly in the evaluator and stamp the applied limit into the trace; caller-supplied values are overrides, never the source of truth.
9. **Absent provider config means unknown, not a fabricated `false`** — when capability/classification evidence is derived from a configured provider and the config is absent, the evidence key must be **omitted** (unknown), never emitted as a definitive negative via a default-object cast (`provider ?? { authMode: undefined }` → `false`). A definitive `false` claims proven absence, bypasses `unknownEvidence` `penalize`/`allow` policies, and can exclude the only supporting candidate. Mirror any authMode/registry backfill the router applies before the same canonical check, so evidence agrees with routing.
10. **Misplaced CLI flags must be rejected, not consumed as positional ids** — a command that `shift()`s its first positional before scanning for flags treats `--json` as the profile id: `show` reports "unknown routing profile: --json", and `dry-run` posts `"--json"` to the API and waits for a 404. Reject an id/positional that starts with `-` immediately after extraction and show usage (both the show and dry-run flows).
11. **Hardcoded capability sets must match reality** — adapter/capability allowlists that are typed against a subset of the real values (e.g. `TOOL_CAPABLE_ADAPTERS` vs actual adapter ids) drift silently; use a shared literal union so invalid ids are rejected, and test the cross-product.
...
1. **One decision, one clock** — a single routing/evaluation decision must read `Date.now()`/`now` once and thread it; multiple clock reads in one call graph are a race (a cooldown expiring between reads produces a candidate that carries `cooldownUntilMs` but is not excluded).
2. **Filter before LIMIT** — a query that applies `LIMIT`/`maxRows` before the candidate/category filter drops samples non-deterministically (sample membership depends on unrelated traffic); filter first, then truncate, and make truncation deterministic.
3. **Aggregate semantics match the doc** — when a comment says "every X or Y", the code must implement that union, not "all X or all Y"; mixed distributions (some in state A, some in state B) must still aggregate.
4. **Byte budgets measure bytes** — `JSON.stringify(x).length` is UTF-16 code units; a documented "≤ 16 KiB" trace must measure encoded bytes, cap strings/arrays to the stated limit, set `truncated` flags when capping, and not copy caller-supplied unknown nested fields verbatim.
5. **Route body limit matches producer budget** — the body-size limit the management route enforces must be ≥ the serialized budget the producer claims; a trace that "fits" its own budget must not exceed the route limit after normalization.
...
3. **Incremental paths stay incremental** — an identity/change check that includes volatile fields (`size`, `mtime`) turns every append into a full destroy-and-rebuild (O(ledger) per query): after any new row, `size`/`mtime` differ, so the identity check fails and the tail-ingest path never runs. Use stable identity (`path`/`dev`/`ino`/`birthtime`) for replacement detection, keep volatile fields only for tail decisions, and let the byte offset handle appended rows. Regression: append one row → indexedRows grows, `lastError` unchanged, no rebuild.
...
5. **DB-result casts must be validated at the boundary** — SQLite (and other drivers) return `null` for NULL columns; an `as HealthSample[]` cast that skips validation lets `null` flow into math typed as `number`. A NULL `duration_ms` becomes a perfect latency score (`1 - null/60_000` → `1`) and a NULL `status` classifies as success (`null >= 400` is `false`). Validate every row: require finite numeric status/timestamp, store non-finite durations as excluded (skip the latency push), never sort or median with `null`.
