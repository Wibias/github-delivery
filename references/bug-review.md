...
8. **Hardcoded capability sets must match reality** — adapter/capability allowlists that are typed against a subset of the real values (e.g. `TOOL_CAPABLE_ADAPTERS` vs actual adapter ids) drift silently; use a shared literal union so invalid ids are rejected, and test the cross-product.
