...
Rule/skill/workflow loading, file reads, Git/GitHub snapshots, remote fetches, obvious command retries, and shell-quoting corrections are not user-facing progress by themselves. Perform them directly. Do not emit status messages whose only new information is “loaded X”, “next I’ll verify Y”, “I’ll lock/fetch/recapture Z”, or that an obvious retry will now be attempted. Before changing quoting, retrying, or reissuing a failed deterministic command, inspect that command's first stdout/stderr result and preserve it as failure evidence; malformed or decorated output is not proof that the command syntax was wrong. If a failed tool call has a deterministic correction and does not change scope or create a blocker, correct it and retry quietly; surface the failure only when it materially changes the plan, evidence, authority, or user action required.
...
When a workflow assigns an analysis axis or scope pass to a specialist/subagent and that result is valid, current, and complete for the required question, the parent consumes and integrates that evidence instead of repeating the same analysis. Parent-side re-analysis is justified only by a material gap, ambiguity, conflicting evidence, or relevant state change.
