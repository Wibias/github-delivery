...
If the current branch conventionally encodes an issue number as its work-item identifier (for example `feat/239-integrations-legacy-cleanup`), treat that number only as a branch-derived issue candidate. Before publication, use read-only GitHub evidence to verify that the issue exists and is open in the same repository, then compare the issue scope with the candidate diff/commit summary and require a material scope match.

If the candidate is missing, closed, ambiguous, or the issue scope does not match the local work, do not link it. Do not search arbitrary commit/body numbers for alternatives. If the candidate is verified, include exactly one `Closes #N` closing reference in the intended PR body before the publication plan is locked. Do not assign or comment on the issue, change its labels/state directly, or add any other issue-side effect unless the user separately requested that action. Keep the local-work route and all of its normal publication gates.
